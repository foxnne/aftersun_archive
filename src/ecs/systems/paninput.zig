...
pub fn process(it: *flecs.ecs_iter_t) callconv(.C) void {
    var inputs = it.column(components.PanInput, 1);
    var i: usize = 0;
    while (i < it.count) : (i += 1) {
        ...
        if (mouse_pos.x < margin or mouse_pos.x > size_w - margin or mouse_pos.y < margin or mouse_pos.y > size_h - margin) {
            inputs[i].direction = zia.math.Direction.find(8, mouse_pos.x - center_pos.x, mouse_pos.y - center_pos.y);
        } else {
            inputs[i].direction = .None;
        }
    }
}
